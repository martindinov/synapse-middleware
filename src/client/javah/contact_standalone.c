...
void printFlush(char* str) {
  printf("%s", str);
  fflush(stdout);
}

void getContactQualityReadings(struct emokit_device* d) {
...
void disconnectHeadsets(struct emokit_device* d) {
  emokit_close(d);
  emokit_delete(d);
}

void connectToHeadsets() {
